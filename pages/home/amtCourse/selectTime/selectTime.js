...
const conf = {
  ...
  onShow: function() {
    ...
    let s2 = day2.getDate();
    let varDate ='calendarConfig.currtDate'
    let varMonth = 'calendarConfig.currtMonth'
    let s3= day2.getMonth() + 1
    this.setData({
      [varDate]: s2,
      [varMonth]:s3
    })
  ...
  backHander: function() {
    ...
    if (this.data.choseTime.date != '' && this.data.choseTime.time !='' ){
      ...
      let amtTime= 'amtClass.amtTime'
      let result = this.data.choseTime.date + ' ' + this.data.choseTime.time
  ...
  whenChangeMonth(e) {
    ...
    let actives = 'calendarConfig.active'
    let day2 = new Date();
    day2.setTime(day2.getTime());
    let s3 = day2.getMonth() + 1

    if (this.data.calendarConfig.currtMonth != e.detail.next.month){
      this.setData({
        [actives]: '!'
      })
    }else{
    ...
    console.log(this.data.calendarConfig.currtMonth)

    console.log(this.data.calendarConfig.active)

    console.log(e.detail.current.month)
